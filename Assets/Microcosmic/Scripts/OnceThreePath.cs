...
public class OnceThreePath : MonoBehaviour
{
    ...
    void Awake(){
        ...
        road4 = 8;
        */
        Debug.Log("获取到的车辆数量分别为:" + road1 + "  " + road2 + "  " + road3 + "  " + road4 + "  ");
    ...
    IEnumerator SendPutRequest(string PutServeUrl)
    {
        ...
        Debug.Log("准备传输的车辆数量分别为:" + road1 + "  " + road2 + "  " + road3 + "  " + road4 + "  ");
        //ist<int> Car_Num = new List<int> { road1, road2, road3, road4 };
        // 创建一个包含这两个值的 JSON 对象
        ToAI requestBody = new ToAI
        {
            PathNum = path,
            Car_Num = new int[] { road1, road2, road3}
        };
